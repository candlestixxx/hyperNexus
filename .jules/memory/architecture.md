...
*   **Rationale:** Ensures the system can run headless (as `hypernexusd`) while multiple clients (CLI, Web, Browser Extension) interact with it via standardized tRPC/Go APIs.
...
The system is being architected to support "AI HyperNexus Enterprise" features:
...
*   **Parity Principle:** HyperNexus must provide 1:1 behavioral and schema parity for tools expected by proprietary models (e.g., Claude Code's `bash`, `read`, `write`).
