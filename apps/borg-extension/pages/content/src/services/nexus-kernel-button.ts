/**
 * Nexus Kernel Button Injection Service
 *
 * Detects supported AI chat websites (Claude.ai, ChatGPT) and injects
 * a "Nexus Kernel" button that attaches the local Borg Kernel to the
 * active conversation session.
 *
 * The button provides:
 * - One-click attachment to the local Borg Kernel (port 4300)
 * - Visual status indicator (connected/disconnected/warming)
 * - Context bridge: sends conversation context to Borg for memory & healing
 */

import { createLogger } from '@extension/shared/lib/logger';

const logger = createLogger('NexusKernelButton');

// ─── Types ───────────────────────────────────────────────────────────────────

export type NexusKernelStatus = 'disconnected' | 'connecting' | 'connected' | 'error';

export interface NexusKernelConfig {
  kernelUrl: string;
  bridgeUrl: string;
  autoAttach: boolean;
...
export interface NexusKernelButtonState {
  status: NexusKernelStatus;
  kernelUrl: string;
...
const NEXUS_KERNEL_BUTTON_ID = 'borg-nexus-kernel-btn';
const NEXUS_KERNEL_STATUS_ID = 'borg-nexus-kernel-status';
const NEXUS_KERNEL_PANEL_ID = 'borg-nexus-kernel-panel';

const DEFAULT_CONFIG: NexusKernelConfig = {
...
const BUTTON_STYLES = `
  ...
  #${NEXUS_KERNEL_BUTTON_ID}:hover {
    background: rgba(56, 189, 248, 0.15);
    border-color: rgba(56, 189, 248, 0.5);
    color: #38bdf8;
  }
  #${NEXUS_KERNEL_BUTTON_ID}.nexus-connected {
    border-color: rgba(52, 211, 153, 0.4);
    background: rgba(52, 211, 153, 0.08);
    color: #6ee7b7;
  }
  #${NEXUS_KERNEL_BUTTON_ID}.nexus-connected:hover {
    background: rgba(52, 211, 153, 0.15);
    border-color: rgba(52, 211, 153, 0.5);
  }
  #${NEXUS_KERNEL_BUTTON_ID}.nexus-error {
    border-color: rgba(248, 113, 113, 0.4);
    background: rgba(248, 113, 113, 0.08);
    color: #fca5a5;
  }
  #${NEXUS_KERNEL_STATUS_ID} {
    width: 8px;
    height: 8px;
    border-radius: 50%;
    background: #64748b;
  ...
  .nexus-connected #${NEXUS_KERNEL_STATUS_ID} {
    background: #34d399;
    box-shadow: 0 0 6px rgba(52, 211, 153, 0.5);
    animation: nexus-pulse 2s ease-in-out infinite;
  }
  .nexus-error #${NEXUS_KERNEL_STATUS_ID} {
    background: #f87171;
  }
  .nexus-connecting #${NEXUS_KERNEL_STATUS_ID} {
    background: #fbbf24;
...
// ─── Nexus Kernel Button Service ─────────────────────────────────────────────

export class NexusKernelButtonService {
  private config: NexusKernelConfig;
  private state: NexusKernelButtonState;
  private button: HTMLElement | null = null;
  private panel: HTMLElement | null = null;
  private pollTimer: ReturnType<typeof setInterval> | null = null;
  private siteConfig: (typeof SUPPORTED_SITES)[SupportedSite] | null = null;

  constructor(config: Partial<NexusKernelConfig> = {}) {
  ...
  async initialize(): Promise<boolean> {
    ...
    if (!site) {
      logger.debug('NexusKernel: unsupported site, skipping injection');
      return false;
    }

    this.siteConfig = SUPPORTED_SITES[site];
    logger.info(`NexusKernel: detected ${SUPPORTED_SITES[site].name}, injecting button`);
    ...
    if (!anchor) {
      logger.warn('NexusKernel: could not find button anchor element');
      return false;
    }
  ...
  private injectStyles(): void {
    if (document.getElementById('borg-nexus-kernel-styles')) return;

    const style = document.createElement('style');
    style.id = 'borg-nexus-kernel-styles';
    style.textContent = BUTTON_STYLES;
    document.head.appendChild(style);
  }

  /** Create and inject the Nexus Kernel button */
  private createButton(anchor: Element): void {
    if (document.getElementById(NEXUS_KERNEL_BUTTON_ID)) return;

    const btn = document.createElement('button');
    btn.id = NEXUS_KERNEL_BUTTON_ID;
    btn.type = 'button';
    btn.title = 'Attach to Nexus Kernel';

    btn.innerHTML = `
      <span id="${NEXUS_KERNEL_STATUS_ID}"></span>
      <span>Nexus Kernel</span>
    `;
    ...
    logger.info('NexusKernel: button injected');
  }
  ...
  /** Attach to the local Nexus Kernel */
  async attach(): Promise<void> {
    ...
    try {
      ...
      logger.info('NexusKernel: attached to kernel', data);
      ...
      logger.warn('NexusKernel: attach failed', err.message);
    }
  }
  ...
  detach(): void {
    ...
    logger.info('NexusKernel: detached');
  }

  /** Register this session with the Borg Kernel */
  private async registerSession(): Promise<void> {
    ...
    try {
      ...
      logger.debug('NexusKernel: session registration failed (non-critical)', err.message);
    }
  }
  ...
  private showPanel(): void {
    ...
    panel.id = NEXUS_KERNEL_PANEL_ID;
    ...
    document.getElementById('nexus-detach-btn')?.addEventListener('click', () => {
      this.detach();
      this.showPanel(); // Toggle panel off
    });

    document.getElementById('nexus-reconnect-btn')?.addEventListener('click', () => {
      this.detach();
      this.attach();
      this.showPanel(); // Toggle panel off
    });
  ...
  private setState(partial: Partial<NexusKernelButtonState>): void {
    ...
    if (this.button) {
      ...
      switch (this.state.status) {
        case 'connected':
          this.button.classList.add('nexus-connected');
          this.button.title = 'Nexus Kernel: Connected';
          break;
        case 'connecting':
          this.button.classList.add('nexus-connecting');
          this.button.title = 'Nexus Kernel: Connecting...';
          break;
        case 'error':
          this.button.classList.add('nexus-error');
          this.button.title = `Nexus Kernel: Error - ${this.state.error}`;
          break;
        default:
          this.button.title = 'Attach to Nexus Kernel';
      }
    }
  }
  ...
  destroy(): void {
    ...
    const styles = document.getElementById('borg-nexus-kernel-styles');
    styles?.remove();
  }
}
...
let instance: NexusKernelButtonService | null = null;

/** Get or create the Nexus Kernel button service */
export function getNexusKernelService(config?: Partial<NexusKernelConfig>): NexusKernelButtonService {
  if (!instance) {
    instance = new NexusKernelButtonService(config);
  }
  return instance;
}

/** Auto-initialize the Nexus Kernel button if on a supported site */
export async function initNexusKernelButton(config?: Partial<NexusKernelConfig>): Promise<boolean> {
  const service = getNexusKernelService(config);
