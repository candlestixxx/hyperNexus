...
describe('resolveUpstreamBases', () => {
  const originalUpstream = process.env.HYPERNEXUS_TRPC_UPSTREAM;

  afterEach(() => {
    if (originalUpstream === undefined) {
      delete process.env.HYPERNEXUS_TRPC_UPSTREAM;
    } else {
      process.env.HYPERNEXUS_TRPC_UPSTREAM = originalUpstream;
    }
  });

  it('includes HyperNexus core\'s default tRPC port before legacy fallbacks', () => {
    delete process.env.HYPERNEXUS_TRPC_UPSTREAM;
  ...
  it('prepends a configured upstream while deduplicating defaults', () => {
    process.env.HYPERNEXUS_TRPC_UPSTREAM = 'http://127.0.0.1:4000/trpc';
...
describe('legacy MCP dashboard compatibility bridge', () => {
  ...
  const originalUpstream = process.env.HYPERNEXUS_TRPC_UPSTREAM;
  const originalHyperNexusConfigDir = process.env.HYPERNEXUS_CONFIG_DIR;
  let compatConfigDir = '';

  beforeEach(async () => {
    compatConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), 'hypernexus-trpc-compat-'));
    process.env.HYPERNEXUS_CONFIG_DIR = compatConfigDir;
  });
  ...
  afterEach(async () => {
    ...
    if (originalHyperNexusConfigDir === undefined) {
      delete process.env.HYPERNEXUS_CONFIG_DIR;
    } else {
      process.env.HYPERNEXUS_CONFIG_DIR = originalHyperNexusConfigDir;
    }
  ...
  it('returns compatibility data for modern MCP procedure batches when upstreams are unavailable', async () => {
    process.env.HYPERNEXUS_TRPC_UPSTREAM = 'http://127.0.0.1:59999/trpc';
    ...
    expect(response.headers.get('x-hypernexus-trpc-compat')).toBe('legacy-mcp-dashboard-bridge');
  ...
  it('probes top-level mcpServers.list when bridging legacy MCP batches', async () => {
    process.env.HYPERNEXUS_TRPC_UPSTREAM = 'http://127.0.0.1:4100/trpc';
  ...
  it('returns local dashboard fallback data for richer MCP pages when upstreams are unavailable', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get('x-hypernexus-trpc-compat')).toBe('local-dashboard-fallback');
    expect(Array.isArray(payload)).toBe(true);
  ...
  it('normalizes batched bulk import payloads before proxying them upstream', async () => {
    process.env.HYPERNEXUS_TRPC_UPSTREAM = 'http://127.0.0.1:3100/trpc';
  ...
  it('supports local pseudo-managed MCP server actions when upstreams are unavailable', async () => {
    ...
    try {
      ...
      expect(listResponse.headers.get('x-hypernexus-trpc-compat')).toBe('legacy-mcp-dashboard-bridge');
      ...
      expect(getResponse.headers.get('x-hypernexus-trpc-compat')).toBe('local-dashboard-fallback');
      ...
      expect(getViaQueryResponse.headers.get('x-hypernexus-trpc-compat')).toBe('local-dashboard-fallback');
      ...
      expect(reloadResponse.headers.get('x-hypernexus-trpc-compat')).toBe('local-mcp-managed-action');
