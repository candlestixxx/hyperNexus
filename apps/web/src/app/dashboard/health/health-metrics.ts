...
export function getMcpRouterMetric(
    ...
    if (startupStatus?.status === 'degraded') {
        return {
            ...
            detail: summary || 'Live startup telemetry is unavailable while HyperNexus serves a compat-fallback router snapshot.',
        };
    }
...
export function getEventBusMetric(startupStatus?: DashboardStartupStatus): {
    ...
    if (startupStatus?.status === 'degraded') {
        return {
            status: 'Degraded',
            color: 'text-amber-500',
            detail: summary || 'Live startup telemetry is unavailable while HyperNexus serves a compat-fallback snapshot.',
        };
    }
