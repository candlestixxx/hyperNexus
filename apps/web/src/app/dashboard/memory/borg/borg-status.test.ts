...
describe('claude-mem status helpers', () => {
    it('summarizes the current HyperNexus claude-mem parity state honestly', () => {
    ...
    it('guides operators when the adapter store has not been created yet', () => {
        expect(getClaudeMemOperatorGuidance({
            ...
            detail: 'No HyperNexus-managed claude_mem store exists yet. When the adapter initializes, it seeds 5 default buckets for project context, user facts, style preferences, commands, and general notes.',
            tone: 'warning',
        });
    });
    ...
    it('warns when claude-mem is not part of the active memory pipeline', () => {
        expect(getClaudeMemOperatorGuidance({
            ...
            detail: 'Core reports the active memory pipeline as json with json. The adapter file can still exist on disk, but HyperNexus is not currently writing new memories through claude-mem.',
            tone: 'warning',
        });
    });
});