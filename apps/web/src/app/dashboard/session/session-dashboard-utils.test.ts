...
describe('session dashboard utils', () => {
    ...
    it('builds a cmd-compatible attach command when compatibility mode selected', () => {
        expect(buildAttachCommand('C:\\Users\\hyper\\workspace\\hypernexus', 'claude.cmd', ['--resume', 'latest'], {
            shellFamily: 'cmd',
            shellPath: 'C:\\Windows\\System32\\cmd.exe',
        })).toBe('cd /d "C:\\Users\\hyper\\workspace\\hypernexus" && "claude.cmd" "--resume" "latest"');
    });

    it('builds a WSL attach command when the selected shell uses WSL', () => {
        const command = buildAttachCommand('/mnt/c/Users/hyper/workspace/hypernexus', 'bash', ['-lc', 'pwd'], {
            shellFamily: 'wsl',
            shellPath: 'wsl',
        });

        expect(command.startsWith('wsl -e sh -lc ')).toBe(true);
        expect(command).toContain('/mnt/c/Users/hyper/workspace/hypernexus');
