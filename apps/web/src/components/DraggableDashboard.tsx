...
import { ActivityPulse, SystemHealth, LatencyMonitor, SecurityWidget } from "@hypernexus/ui";
...
export default function DraggableDashboard() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('hypernexus_dashboard_layout');
        if (saved) {
            try {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            setItems((items) => {
                ...
                localStorage.setItem('hypernexus_dashboard_layout', JSON.stringify(newOrder));
                return newOrder;
            });
        }
