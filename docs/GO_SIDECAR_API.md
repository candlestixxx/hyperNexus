...
> **Context**: The `go/` workspace is an experimental sidecar and coexistence lane designed for operator visibility and read-parity. It currently runs alongside the primary Node.js control plane (`hypernexusd`).
...
* **`/api/runtime/locks`**: Scans the `.hypernexus` directory for active `hypernexus-go` and Node.js lock files.
...
These endpoints exist to maintain API parity for clients connecting to the Go sidecar on port `4000`, but they strictly proxy to the upstream Node.js TRPC control plane (default `3000` or defined in `.hypernexus_startup_marker`). If the Node process is offline, these routes will return `503 Service Unavailable`.
...
2. **Writes/Execution**: ALWAYS implement as a Bridge-Only Passthrough to the Node control plane unless specifically migrating ownership of a daemon layer (e.g., `hypernexusingest`). Do not duplicate execution logic in Go while the Node daemon still owns it.
