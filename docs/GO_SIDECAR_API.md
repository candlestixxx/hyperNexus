...
2. **Writes/Execution**: ALWAYS implement as a Bridge-Only Passthrough to the Node control plane unless specifically migrating ownership of a daemon layer (e.g., `hypercodeingest`). Do not duplicate execution logic in Go while the Node daemon still owns it.
