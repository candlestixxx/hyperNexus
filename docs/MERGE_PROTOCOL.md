...
> Standardized procedure for handling upstream forks, submodule updates, and feature integrations in the hypernexus monorepo.
...
**Resolve conflicts carefully** — prefer the hypernexus-side implementation when in doubt. If upstream merges fail with "unrelated histories," identify the exact reason and formulate a fallback strategy (cherry-pick, manual merge, or archive).
