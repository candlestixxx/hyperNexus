# hypernexus 0.9.1: Project Roundtable Brief

## Introduction
This document serves as the master briefing for all AI models (Claude 3.7 Sonnet, GPT-4o, Gemini 2.5 Pro, and others) evaluating, debating, or developing the hypernexus Control Plane. It outlines the absolute state of the repository, what works, what is planned, and our philosophical boundaries.

## 1. The Core Philosophy
hypernexus is a **Cognitive Control Plane** for AI agents. It sits between the agent (the brain) and the infrastructure (the tools, the files, the commands). 
...
- **Browser Extension**: `apps/hypernexus-extension` is a 1:1 replacement for the legacy MCP-SuperAssistant. It captures web pages automatically and exposes the browser DOM to hypernexus via WebSocket.
...
4. **Universal Agent Attach**: The ability to seamlessly attach hypernexus to a running `claude` CLI or `cursor` editor session and inject memory directly into their prompts.
...
Models are fine-tuned on specific tools (e.g., Claude Code's `bash`, `glob`, `str_replace_editor`). hypernexus intercepts these exact schemas and routes them to our secure implementations. **We do not alter these signatures.** A model running on hypernexus should feel like it is running in its native training environment, but with superpowers.
