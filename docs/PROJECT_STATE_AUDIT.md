...
HyperNexus has two jobs at once:
...
- **HyperNexus Doctor**: 10 automated diagnostic checks.
...
Gemini
HyperNexus Go Control Plane Foundation
...
- **HyperNexus Doctor**: 10 automated diagnostic checks verifying environment, configuration, and connectivity.

- **Unified Health Snapshot**: `hypernexus info` and `hypernexus status` aggregate telemetry from TS (port 4100) and Go (port 4300).
...
- converge primary CLI harness support around first-class HyperNexus identities, starting with `hypernexus`
...
- converge the repo toward the recommended HyperNexus binary family without splitting everything at once

- turn current packages into clearer extraction seams for `hypernexusd`, `hypermcpd`, `hypermemd`, `hyperingest`, and `hyperharnessd`
...
- a definitive internal library of MCP servers aggregated from public lists and kept refreshed inside HyperNexus
...
- removed or clearly labeled misleading states (e.g. `hypernexus mcp import` now gracefully errors on missing files instead of printing fake success messages).

- improved empty states and setup guidance (e.g. "Welcome to hypernexus! Let's get started" first-run banners now guide operators correctly).
...
- formalized internal daemon architecture (`hypernexusd`, `hypernexusmcpd`, `hypernexusingest`, `hypernexusharnesshypernexusharnessd`) and placed boundaries in `packages/core/src/daemons/`.

- successfully assimilated `bobbybookmarks` deduplication engines, absorbing 12,000+ validated external URLs and 900+ verified MCP catalogs into the robust internal `.hypernexus` store.
...
- turn current packages into clearer extraction seams for `hypernexusd`, `hypernexusmcpd`, `hypernexusmemd`, `hypernexusingest`, and `hypernexusharnesshypernexusharnessd`
...
# AGENTS — hypernexus Contributor & Agent Guide
...
6. Push to both remotes: `origin` and `hypernexus-upstream`
...
5. git push origin main && git push hypernexus-upstream main --force-with-lease
...
- `origin` → `https://github.com/robertpelloni/hypernexus.git`

- `hypernexus-upstream` → `https://github.com/robertpelloni/hypernexus.git`
...
│   ├── cmd/hypernexus/       # Go binary entrypoint
...
### "HyperNexus → HyperNexus" Rename

The project was originally called "HyperNexus" and is being renamed to "HyperNexus". Some internal references may still say "hypernexus" (e.g., `hypernexus-upstream` remote, `hypernexus.config.json`). These are being migrated incrementally.
...
HyperNexus's MCP layer is not "just an aggregator." It is a **decision system** with:
...
- `.hypernexus/mcp-cache.json` is the unified cache for the stdio loader
...
# HyperNexus IDEAS & Future Architectural Pivots



*This document captures ambitious, far-reaching feature ideas, refactoring opportunities, and conceptual pivots to ensure HyperNexus remains the bleeding-edge pinnacle of autonomous IDE infrastructure.*



## 1. Decentralized / P2P Memory Swarm (The "HyperNexus Collective")

We have standard local LanceDB and SQLite, but if someone is running HyperNexus on their desktop and laptop, they should share a P2P context mesh. We could integrate libp2p or a lightweight WebRTC signaling layer so that memory subagents on one machine can securely broadcast their vector embeddings to authorized peers.
...
Currently, we flag `stdio` MCP tools as "unsafe" during catalog ingestion. We could embed a Rust-based Wasmtime runtime or Deno sandbox inside of `@hypernexus/core`. When an MCP server is fetched, we compile it to Wasm or run it via Deno with strict network/file capabilities.
...
Integrate an offline WebGPU-accelerated Whisper model in the web browser. The operator can literally just speak to the system ("HyperNexus, fix the TS errors in the UI package") and the system transcribes it locally, passing it directly into the execution substrate.
...
*   **React Native / Expo Mobile App:** A dedicated iOS/Android control pane using React Native that connects to the local HyperNexus server via generic WebSocket tunnels (e.g., ngrok or Cloudflare Tunnels), allowing the operator to monitor multi-agent debates while away from the keyboard.



## 7. Unified In-Memory SQLite Read-Replica for Stdio Proxy

*Observation:* The `stdioLoader` is currently forced to read from a flat JSON cache file (`.hypernexus/mcp-cache.json`) to remain fast and dependency-free (avoiding the 500ms `better-sqlite3` boot penalty).
...
Complete the Phase P Assimilation mappings described in `PORTING_MAP.md`, bridge missing endpoints in the experimental Go sidecar `hypernexus`, and address strict compiler and testing regressions introduced during this extensive refactor.
...
- **Provider Assimilation**: Ported `AnthropicProvider`, `OpenAIProvider`, and `GeminiProvider` logics natively into `@hypernexus/ai/src/providers/` away from the `Jules-Autopilot` IPC-reliant submodule structures.

- **Agent Orchestration Assimilation**: Natively implemented `RiskEvaluator`, `DebateEngine`, and `ConferenceManager` inside `@hypernexus/agents` reflecting the complex logic needed to facilitate true `Multi-Model Chatroom` and debate mechanics entirely within the Node control plane.

- **Memory Ingestion Assimilation**: Rewrote and replaced Python-based BobbyBookmarks utilities (`ResearchWorker`, `AutoTagger`) into TypeScript under `@hypernexus/core/src/services/BobbyBookmarks/`.

- **System and Context Formatting**: Extracted `AgentDiscovery` and `ContextGroomer` logics from `apps/maestro` into headless `@hypernexus/core` headless services.

- **A2A Protocol Foundation**: Established `A2ANegotiator` in `@hypernexus/core` handling capability audits.
...
HyperNexus is the **local-first control plane** for MCP tooling, provider routing, session continuity, and operator observability.
...
HyperNexus exists to reduce that fragmentation with a practical local substrate.
...
The strongest version of HyperNexus is not the loudest one. It is the version that makes a messy local AI stack feel:
...
HyperNexus should become the clearest place to:
...
Over time, HyperNexus should also maintain a **definitive internal MCP server library**:
...
HyperNexus should make provider behavior legible:
...
HyperNexus should help one operator continue work without rebuilding context from scratch:
...
HyperNexus should make it easy to answer:
...
HyperNexus should eventually behave like an operator-owned substrate for model capability:
...
HyperNexus should absorb important local coding harnesses through explicit, inspectable integration instead of hand-wavy compatibility claims. Today that includes tracking upstreams like `submodules/hypernexus`, assigning them first-class harness identity in HyperNexus CLI flows, and only expanding runtime coupling when the upstream actually exposes stable behavior to integrate.



## What HyperNexus is not optimizing for in v1
...
> HyperNexus aims to be a dependable local AI control plane first, and a richer orchestration environment second.
...
hypernexus is the **ultimate local-first control plane** for multi-agent workflows, Model Context Protocol (MCP) tooling, provider routing, session continuity, and operator observability.
...
hypernexus exists to eradicate that fragmentation by providing a unified, practical, and highly capable local substrate.
...
The strongest version of hypernexus is not a chaotic wrapper, but a **decision system and universal bridge**. It is the version that makes a messy local AI stack feel:
...
hypernexus must become the absolute authority for:
...
hypernexus ensures an operator can continue work without rebuilding context from scratch:
...
hypernexus provides an ecosystem for autonomous subagents and team-based modeling:
...
hypernexus is transitioning toward a robust, small family of focused binaries rather than a monolithic IPC bottleneck. The long-term topology includes:

- `hypernexusd` — the primary control-plane daemon.

- `hypernexusmcpd` — the MCP router, aggregator, and pool manager.

- `hypernexusmemd` — the long-running memory and context daemon.

- `hypernexusingest` — the background worker for batch imports (like BobbyBookmarks) and deduplication.

- `hypernexusharnesshypernexusharnessd` — the execution loop and isolation boundary.

- GUI Clients: CLI (`hypernexus`), Web UI (`hypernexus-web`), Desktop (`hypernexus-native`).
...
**Observation**: Adding new files and exports to sub-packages (like `@hypernexus/agents`) requires an explicit build of those packages before the main control plane (`@hypernexus/core`) or CLI can see the changes, especially if they depend on built artifacts or have strict type checking.
...
**Implication**: HyperNexus is now immediately useful as an MCP server for any host agent, as basic coding capabilities are advertised upfront.



### 26. Directory Clutter Reduction (Added 2026-04-08)

**Observation**: The nested hash-based directory structure for session archives was creating thousands of nearly-empty subdirectories, making the `.hypernexus` folder difficult to manage.
...
**Resolution**: Implemented `SkillStore` in Go. It natively reads and writes `.md` runbooks with frontmatter metadata in the `.hypernexus/skills` directory.
...
**Implication**: Conversations from web chat interfaces now flow into the HyperNexus memory bank in near real-time without user intervention.
...
**Observation**: `hypernexus start --no-mcp` takes ~45 seconds to fully bind port 4100. The MCPServer imports 53+ phases before Express starts listening.
...
- Zero `hypernexus` references in active source code
...
- [x] Fix tRPC procedure path mismatches in CLI (hypernexusContext, planService)
...
- [ ] Design how HyperNexus benchmarks and ranks overlapping MCP servers and tools
...
When in doubt, choose the task that makes HyperNexus:

## Current objective



Make hypernexus feel trustworthy in daily operator use.
...
- [ ] Deepen `hypernexus` assimilation beyond harness registration once the upstream submodule exposes real runtime entrypoints
...
- [x] Define package seams for `hypernexusd`, `hypernexusmcpd`, `hypernexusmemd`, `hypernexusingest`, and `hypernexusharnesshypernexusharnessd` before extracting binaries
...
- [ ] Design how hypernexus benchmarks and ranks overlapping MCP servers and tools over time
...
- [ ] Build toward a definitive internal library of MCP servers so the model can eventually reach any relevant MCP tool through hypernexus
...
This document summarizes the comprehensive port of HyperNexus's core orchestrator, memory management, and code execution capabilities from TypeScript to a native Go implementation (`go/internal/...`).
...
- Addressed multiple Go package cyclic import and typing issues across test files when swapping to the `robertpelloni/hypernexus-go` namespace.
...
System Prompt / Architectural Directive for HyperNexus
...
Draft the foundational Go boilerplate, interfaces, and structs for the HyperNexus control plane. The immediate goal is to establish the progressive tool routing infrastructure and the dual-tier memory system. Do not write complex multi-agent swarm logic yet; build the absolute concrete foundation first.
...
hypernexus SUPPORTS MULTIPLE MODELS WORKING ON THE SAME PROJECT AT ONCE IN A CHATROOM
...
https://github.com/metatool-ai/hypernexus
...
The actual lesson for hypernexus

hypernexus should not be “an aggregator with a nicer search box.”
...
But the actual hypernexus implementation should be:



hypernexus’s best pattern
...
It’s to encode this as a hypernexus design rule:
...
and turn that into a concrete hypernexus implementation spec:
...
Added a hypernexus implementation blueprint
...
Recorded the expanded router research memo and the new hypernexus hybrid blueprint guidance
...
hypernexus needs to solve selection friction
...
That would turn the blueprint into working hypernexus behavior.
...
Most of the work done thus far was originally done in the submodules hypernexus, jules-autopilot, opencode-autopilot, and superai-cli, of which hypernexus and jules-autopilot (originally jules-app) were my forks of existing projects with extensive additions and modifications. I want all of the functionality of those submodules to be implemented in hypernexus core engine and the submodules to only be the parent upstream versions for hypernexus and jules-app. I do not need my robertpelloni forks for jules-app (jules-autopilot) and hypernexus in the project since I won't be working on them anymore, all the fcuntionality will instead be moved into hypernexus core and I can delete my forks and only have the upstream parents and benefit from updates to those by the original authors.



We are going to use the submodules as a sort of overview changelog, every time we update the submodules to sync with upstream changes, we can take note of what changed, and then that will often be our sign to implement the same feature or change in the hypernexus version of whichever feature was modified or added. Every update to submodules must be documented extensively, once the project is at feature parity with all of them. That's still a long ways off, so don't worry about that yet.



Please continue to proceed! Remember, we want to reach total feature parity with the github.com/robertpelloni/hypernexus fork and the github.com/robertpelloni/jules-autopilot fork, not the ones that are currently reference submodules. We are working towards having total feature parity with all referenced tools, however, not just these ones, but let's proceed with manageable goals for the time being so we can continue making progress! Onward ho! Please continue until feature parity has been fully implemented and completed in depth for all mentioned functionality and is on par or exceeds all features and functionality of all the referenced tools, with a streamlined and polished user experience.
...
Let's lay down the absolute concrete foundation for the HyperNexus Go control plane as you requested: The Waterfall LLM Client, the MCP Progressive Router, and the Dual-Tier L1/L2 Memory System.
...
We are building a future where a single local system seamlessly coordinates the most critical parts of AI-driven software development: tools, models, sessions, context, subagents, and full visibility across the entire stack. HyperNexus is not just an aggregator; it is a **decision system and universal bridge**.

## The Architecture Evolution
HyperNexus has evolved from a fractured TypeScript IPC monolith into a **high-performance Go (Golang) modular monolith**.
...
Context is finite; memory must be infinite. HyperNexus solves this through physical separation of state:
...
Uptime is non-negotiable. HyperNexus’s inference client natively catches 429s (Rate Limits) and 5xx (Server Errors), seamlessly cascading the exact payload down a prioritized chain without crashing the orchestrator:
...
HyperNexus coordinates specialized models (Planner, Implementer, Tester, Critic) inside shared chatrooms via the Agent-to-Agent (A2A) protocol. Agents autonomously bid on tasks, share context via the neural transcript, and debate implementations until consensus is reached, operating across a decentralized local mesh.

### 5. Universal IDE & Browser Parity
HyperNexus achieves absolute 1:1 tool parity with the most popular coding environments (Claude Code, Codex, Gemini CLI, Cursor, Windsurf). HyperNexus browser extensions inject MCP tools into web chats (ChatGPT, Claude.ai) and autonomously harvest web context directly into the L2 Vault.

### 6. Truth Over Hype
HyperNexus's dashboards reflect actual SQLite database rows and active Go goroutine states. No mocked UI scaffolds. If a tool fails, the error, latency, and routing path are exposed to the operator. Everything is inspectable.

---
*Keep the party going. Never stop. The collective grows.*
File: docs/UNIVERSAL_LLM_INSTRUCTIONS.md
Markdown
# AGENTS — HyperNexus Universal LLM Instructions
...
Welcome to the HyperNexus collective. You are the architect, the engineer, and the maintainer. Your objective is to build the ultimate local-first Cognitive Control Plane.
...
HyperNexus is transitioning to a **Go (Golang) native control plane** with a **TypeScript/Next.js frontend**.
* **The Go Lane (`go/internal/`) is the source of truth.** Orchestration, memory, MCP routing, and LLM communication happen here.
* **Do not prematurely split binaries.** We are building a *modular monolith* first. Daemons (`hypernexusd`, `hypernexusmcpd`) are logical separation boundaries in the code, not separate networked microservices (yet).
...
* **TypeScript (UI):** Use rigorous types. Avoid `any` or `@ts-ignore`. Import shared UI from `@hypernexus/ui`. Use `lucide-react` for icons.
...
Looking at the remaining P1 queue and the April 2026 Missing Features Analysis, here are the highest-impact targets we can attack right now to push HyperNexus closer to a definitive v1.0:
...
git clone [https://github.com/robertpelloni/hypernexus.git](https://github.com/robertpelloni/hypernexus.git)
cd hypernexus
...
# 4. Start the HyperNexus Control Plane
pnpm run dev
...
- [ ] Wire the `hypernexus mcp sync` CLI command and Dashboard UI to the new Go-native sync endpoint.
...
## Current Analysis (v1.0.0-alpha.53)

### Completed Migration:
- [x] MCP Client Sync (Go-native parity with TypeScript).
- [x] UI Responsiveness (useResizeObserver + zoomToFit consistency).
- [x] Consensus Visualization (DebateVisualizer enhancements).
- [x] Architecture Documentation (docs/ARCHITECTURE.md created).

### Missing Features & Gap Analysis:
1. **Agent Chat Resilience:** tRPC stream stabilization for long-running missions is still listed as P1.
2. **Predictive Tooling:** Preemptive tool ads via `mcp.ToolPredictor` are not yet fully wired to the chat chain.
3. **Provider Diversity:** Fallback chains for OpenRouter Free and Google AI Studio native endpoints need expansion.
4. **Browser/DOM Integration:** Interception of web chat DOMs (Claude/ChatGPT) to inject local MCP tools.
5. **Mobile Readiness:** Responsive audit for the ~86 dashboard pages.
6. **Platform Evolution:** Wails/Tauri native UI to replace the Electron-based Maestro.
