...
This document maps the current structure of the HyperNexus/HyperNexus repository, shows where the main modules live, and explains how the major pieces connect.
This document maps the current structure of the hypernexus/hypernexus repository, shows where the main modules live, and explains how the major pieces connect.
...
| --- | --- | --- |
| CLI orchestrator | `packages/cli` | Main local terminal entrypoint (`hypernexus`) |
| CLI orchestrator | `packages/cli` | Main local terminal entrypoint (`hypernexus`) |
...
| `apps/hypernexus-extension` | Browser extension application | Exists in repo but excluded from root `pnpm-workspace.yaml` |
| `apps/hypernexus-extension` | Browser extension application | Exists in repo but excluded from root `pnpm-workspace.yaml` |
| `apps/vscode` | VS Code extension app | Exists in repo but excluded from root `pnpm-workspace.yaml` |

### How `apps/` connect

- `apps/web` depends directly on `@hypernexus/core` and `@hypernexus/ui`
- `apps/web` depends directly on `@hypernexus/core` and `@hypernexus/ui`
...
- `@hypernexus/core` for control-plane access and route integration
- `@hypernexus/ui` for shared components

`packages/ui` is the shared component layer used to avoid duplicating operator UI across apps. In this repo, the documented convention is that `apps/web` should import shared UI from `@hypernexus/ui`.

## 9. `packages/cli`

`packages/cli` is the main CLI lane and publishes the `hypernexus` command.

It depends on `@hypernexus/core`, so the CLI is not a separate backend; it is another surface over the same TypeScript control-plane concepts.
- `@hypernexus/core` for control-plane access and route integration
- `@hypernexus/ui` for shared components
...
- module: `github.com/robertpelloni/hypernexus-go`
- module: `github.com/hypernexushq/hypernexus-go`
- top-level layout:
...
- the primary HyperNexus/HyperNexus workspace at the root
- the primary hypernexus/hypernexus workspace at the root
...
| Path | Purpose |
| --- | --- |
| `submodules/hypernexus` | Experimental external CLI harness/upstream assimilation lane |
| `submodules/hypernexus` | Experimental external CLI harness/upstream assimilation lane |
...
- persistent local artifacts such as `data/bobbybookmarks`, `hypernexus.db`, and `cipher-sessions.db`
- persistent local artifacts such as `data/bobbybookmarks`, `hypernexus.db`, and `cipher-sessions.db`
...
| CLI orchestration | `packages/cli` | `packages/core`, `submodules/hypernexus` |
| CLI orchestration | `packages/cli` | `packages/core`, `submodules/hypernexus` |
...
| Editor and extension bridges | `packages/vscode`, `packages/browser-extension`, `packages/jetbrains`, `packages/zed-extension`, `apps/vscode`, `apps/hypernexus-extension` | `packages/core` |
| Editor and extension bridges | `packages/vscode`, `packages/browser-extension`, `packages/jetbrains`, `packages/zed-extension`, `apps/vscode`, `apps/hypernexus-extension` | `packages/core` |
