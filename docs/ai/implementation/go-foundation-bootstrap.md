...
- `foundation/assimilation/inventory.go`
  - upstream assimilation inventory covering imported toolchains and HyperNexus
...
- `agent/agent.go`
  ...
  - system prompt now incorporates HyperNexus/HyperNexus and provider adapter context
...
- `foundation/adapters/hypernexus.go`
  - first HyperNexus/HyperNexus adapter seam for the Go foundation
  - exposes assimilation status, memory context, provider status, MCP config visibility, and adjacent HyperNexus repo discovery
...
- `agent/agent_test.go`
  ...
  - verifies HyperNexus adapter presence on the top-level agent
...
- HyperNexus/HyperNexus adapter seam tests
...
3. expand `foundation/adapters` from visibility and route-selection seams into richer HyperNexus/HyperNexus provider routing and richer MCP execution adapters,
