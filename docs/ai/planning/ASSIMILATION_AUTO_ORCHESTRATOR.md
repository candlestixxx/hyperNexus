# Planning: Assimilation of OpenCode Autopilot (hypernexus Auto-Orchestrator)

## 🌌 Overview
This document outlines the plan to assimilate `github.com/robertpelloni/opencode-autopilot` into the hypernexus monorepo as a first-class orchestration layer. The assimilated project will be renamed to **hypernexus Auto-Orchestrator** and will provide multi-model council supervision for autonomous agent workflows.

## 🏗️ Target Architecture

### Package Information
- **Name**: Integrated directly into `@hypernexus/core`
...
2. **Supervisor Adapters**: Pluggable adapters for OpenAI, Anthropic, Gemini, etc. (to be aligned with `@hypernexus/core` providers).
...
The Auto-Orchestrator will consume `@hypernexus/core` for:
...
A new "Council" section will be added to `@hypernexus/web` (`apps/web`):
...
### 3. API & Communication
- Endpoints currently in `opencode-autopilot` (Hono) will be migrated to tRPC procedures in `@hypernexus/core` or a new router in the orchestrator package.
- Real-time updates will continue using WebSockets, likely unified under the main hypernexus socket.

## 🚀 Phases

### Phase 1: Preparation (Active)
- [x] Analyze source repository (`opencode-autopilot`).
- [x] Define package structure (decided to integrate into `@hypernexus/core`).
...
- [x] Update internal imports to use relative paths instead of `@hypernexus-orchestrator/shared`.

### Phase 3: Core Logic Migration (In Progress)
- [ ] Refactor `CouncilService` and `ConsensusEngine` to use hypernexus primitives.
- [ ] Re-implement Supervisor adapters to use hypernexus's common provider logic.
- [ ] Migrate Hono/Bun routes to Express/tRPC.
- [ ] Integrate with hypernexus's SQLite database.

### Phase 4: Interface & Wiring
- [ ] Expose council triggers as MCP tools in `@hypernexus/core`.
...
- **Consistency**: Use the same coding standards, linting, and formatting as the rest of hypernexus.
