...
## Objective
Make the Go runtime the **primary HyperNexus control plane** and port **every backend feature/function** from the current TypeScript runtime into Go, leaving only the most crucial TypeScript pieces in place temporarily:
- UI clients
...
### Exit criteria
- `start.bat` and `hypernexus start` prefer Go by default
...
- the CLI Go runtime launcher now prefers the prebuilt `go/hypernexus(.exe)` binary and only falls back to `go run ./cmd/hypernexus` when the binary is absent or source launch is explicitly forced
...
- `hypernexus status` now exposes persisted startup provenance from the local startup lock when available
...
- the default HyperNexus startup path runs Go first
