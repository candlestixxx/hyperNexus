...
## Problem
The current repository has an aspirational Go CLI and orchestration shell, but the implementation is still mostly stub-level and does not yet provide truthful parity with Pi, OpenCode, Aider, Goose, Factory Droid, Grok CLI, HyperNexus, or the other imported submodules.
...
6. HyperNexus/HyperNexus-native integration for MCP routing, memory, provider routing, and context continuity.
...
- HyperNexus/HyperNexus-backed MCP tool surfaces exposed through stable adapters
...
### 3. HyperNexus as substrate, not competitor
The new harness should not reimplement HyperNexus’s core control-plane responsibilities unless there is a compelling reliability reason.

HyperNexus should remain the preferred substrate for:
- MCP aggregation
