# HyperNexus Core Assimilation Guide

## System Overview
`cli-orchestrator` has undergone a massive architectural overhaul to transition from a standalone monolithic orchestration tool into a resilient, self-healing, distributed sub-component ready for direct assimilation into the **HyperNexus Core**.

## Core Capabilities for HyperNexus

### 1. Distributed PTY Sidecar Architecture (The "HyperNexus-Harness")
# hypernexus Core Assimilation Guide
...
### 6. Collective Memory
A persistent SQLite "Knowledge Base" is maintained by the `CollectiveMemoryService`. Any sub-agent across any session can store and retrieve facts, sharing knowledge globally across the HyperNexus hive-mind.
...
- `SelfEvolutionService` adjusts supervisor trust weights dynamically based on historical debate outcomes and can spawn high-privilege meta-sessions to rewrite the HyperNexus orchestration source code.

## Assimilation Next Steps
1. **Repository Merge:** Move the `cli-orchestrator` package into the HyperNexus Core monorepo structure.
2. **Global Telemetry:** Hook the `wsManager` broadcast events to the HyperNexus Core global telemetry bus.
3. **Database Unification:** Migrate the `bun:sqlite` implementation to HyperNexus's central high-availability datastore if required.
4. **Service Discovery:** Replace hardcoded `127.0.0.1` PTY ports with HyperNexus Core's dynamic service mesh/discovery network.
A persistent SQLite "Knowledge Base" is maintained by the `CollectiveMemoryService`. Any sub-agent across any session can store and retrieve facts, sharing knowledge globally across the hypernexus hive-mind.
