...
func TestResolveTRPCBasesUsesConfiguredExclusivelyWhenSet(t *testing.T) {
	...
	// Seed a lock file — this should be IGNORED when HYPERNEXUS_TRPC_UPSTREAM is set
	...
	t.Setenv("HYPERNEXUS_TRPC_UPSTREAM", "http://127.0.0.1:4200/trpc")
	bases := ResolveTRPCBases(mainLockPath)
	if len(bases) != 1 {
		t.Fatalf("expected exactly 1 base when HYPERNEXUS_TRPC_UPSTREAM is set, got %v", bases)
	}
...
func TestResolveTRPCBasesUsesLockfileAndDefaultsWhenNoEnv(t *testing.T) {
	...
	// Ensure HYPERNEXUS_TRPC_UPSTREAM is not set
	t.Setenv("HYPERNEXUS_TRPC_UPSTREAM", "")
	bases := ResolveTRPCBases(mainLockPath)
	if len(bases) < 2 {
...
func TestCallTRPCProcedureReturnsUnwrappedJSONData(t *testing.T) {
	...
	defer server.Close()
	t.Setenv("HYPERNEXUS_TRPC_UPSTREAM", server.URL+"/trpc")
