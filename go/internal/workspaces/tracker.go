...
// ~/.hypernexus/workspaces.json, verifying they still exist on disk.
package workspaces
...
// If registryPath is empty, defaults to ~/.hypernexus/workspaces.json.
func NewWorkspaceTracker(registryPath string) *WorkspaceTracker {
	if registryPath == "" {
		...
		registryPath = filepath.Join(home, ".hypernexus", "workspaces.json")
	}
