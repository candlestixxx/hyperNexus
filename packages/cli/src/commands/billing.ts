/**
 * `hypernexus billing` — Provider quotas, usage tracking, and model pricing
 */
...
export function registerBillingCommand(program: Command): void {
  ...
  billing
    ...
    .action(async (opts) => {
      ...
      if (quotas.length === 0) {
        console.log(chalk.dim('  No quota data. Configure providers with `hypernexus provider add`.\n'));
        return;
      }
  ...
  billing
    ...
    .action(async (opts) => {
      ...
      if (Array.isArray(providers) && providers.length > 0) {
        ...
        console.log(chalk.dim('  Use: hypernexus billing fallback --set openai anthropic google'));
      }
      console.log('');
    });
