/**
 * `hypernexus dashboard` - Open the web dashboard
 *
 * Launches the HyperNexus WebUI dashboard in the default browser.
 * If the server isn't running, optionally starts it first.
 *
 * @example
 *   hypernexus dashboard            # Open dashboard in browser
 *   hypernexus dashboard --port 8080
 */
...
export function registerDashboardCommand(program: Command): void {
  program
    .command('dashboard')
    .alias('ui')
    .description('Open the HyperNexus WebUI dashboard in the default browser')
    ...
    .addHelpText('after', `
The dashboard provides a comprehensive visual interface to all HyperNexus subsystems:
...
Examples:
  $ hypernexus dashboard                  Open in browser at localhost:3000
  $ hypernexus dashboard --port 8080      Custom port
  $ hypernexus dashboard --dev            Development mode with HMR
  $ hypernexus dashboard --no-open        Start without opening browser
    `)
    .action(async (opts) => {
      ...
      if (!coreReachable) {
        ...
        console.log(chalk.dim('    Start it with: hypernexus start --port 4100'));
        console.log('');
      }
  ...
  program
    ...
    .action(async (opts) => {
      ...
      const pkgs = [
        ['@hypernexus/core', 'Backend server, MCP router, orchestrator'],
        ['@hypernexus/cli', 'Command-line interface'],
        ['@hypernexus/types', 'Shared TypeScript types & Zod schemas'],
        ['@hypernexus/ai', 'LLM service, model selector'],
        ['@hypernexus/agents', 'Director, Council, Supervisor'],
        ['@hypernexus/tools', 'File, terminal, browser, chain executor'],
        ['@hypernexus/search', 'Semantic & text search service'],
        ['@hypernexus/memory', 'Multi-backend memory system'],
        ['@hypernexus/adk', 'Agent Development Kit'],
      ];
      ...
      // Quick-start hints
      console.log(chalk.bold.cyan('\n  Quick Start:'));
      console.log(chalk.dim('    hypernexus start                # Launch the control plane'));
      console.log(chalk.dim('    hypernexus info                 # System overview'));
      console.log(chalk.dim('    hypernexus provider test openai  # Verify API keys'));
      console.log(chalk.dim('    hypernexus catalog search memory # Browse MCP servers'));
      console.log(chalk.dim('    hypernexus dashboard --dev       # Launch Web UI\n'));
    });
}
