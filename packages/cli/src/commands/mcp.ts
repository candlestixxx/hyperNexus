/**
 * `hypernexus mcp` - MCP Router management commands
 *
 * Comprehensive MCP server lifecycle management, tool browsing,
 * traffic inspection, config sync, and directory access.
 *
 * @example
 *   hypernexus mcp list                    # List all MCP servers
 *   hypernexus mcp start filesystem        # Start a specific server
 *   hypernexus mcp traffic                 # Watch live MCP traffic
 *   hypernexus mcp install @anthropic/mcp  # Install from directory
 *   hypernexus mcp sync                    # Sync configs to all AI tools
 */
...
export function registerMcpCommand(program: Command): void {
  ...
  mcp
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const pidDir = path.join(process.env.HOME ?? '', '.hypernexus', 'mcp-pids');
      ...
      if (servers.length === 0) {
        ...
        console.log(chalk.dim('  No servers found.' + (servers.length === 0 ? ' Is the server running? Use `hypernexus start`.' : '') + '\n'));
        return;
      }
        ...
        try {
          ...
          if (listRes.ok) {
            ...
            if (server?.config?.command) {
              ...
              console.log(chalk.dim(`    Use hypernexus mcp list to see available servers`));
            }
          }
        } catch (e: any) {
    ...
    .action(async (name) => {
      ...
      if (!stopped) {
        try {
          const fs = await import('fs');
          const path = await import('path');
          const pidFile = path.join(process.env.HOME ?? '', '.hypernexus', 'mcp-pids', `${name}.pid`);
  ...
  mcp
    ...
    .option('--auto-start', 'Auto-start on HyperNexus launch', true)
    .addHelpText('after', `
Examples:
  $ hypernexus mcp add filesystem npx -- -y @modelcontextprotocol/server-filesystem /home
  $ hypernexus mcp add github npx -- -y @modelcontextprotocol/server-github --env GITHUB_TOKEN=xxx
  $ hypernexus mcp add remote-api http://localhost:8080/mcp -t streamable-http
    `)
  ...
  mcp
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        console.log(chalk.dim('  Server not reachable. Use hypernexus start to launch.'));
      }
    });
  ...
  mcp
    ...
    .addHelpText('after', `
Examples:
  $ hypernexus mcp install @modelcontextprotocol/server-filesystem
  $ hypernexus mcp install --pip mcp-server-sqlite
  $ hypernexus mcp install --github anthropics/mcp-servers
    `)
    .action(async (pkg, opts) => {
      ...
      try {
        // Add to HyperNexus MCP config via tRPC
        ...
        if (res.ok) {
          ...
          console.log(chalk.dim(`    Use 'hypernexus mcp start ${serverName}' to connect`));
        } else {
          const json = await res.json().catch(() => ({}));
          console.log(chalk.red(`  ✗ Failed: ${json.error?.message ?? res.statusText}`));
        }
        ...
        console.log(chalk.red(`  ✗ Error: ${e.message}`));
        console.log(chalk.dim('    Is the server running? Use hypernexus start'));
      }
    });
  ...
  mcp
    ...
    .option('-o, --output <file>', 'Output file path', 'hypernexus-mcp-export.json')
    .action(async (opts) => {
      const chalk = (await import('chalk')).default;
  ...
  vscode
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        console.log(chalk.red(`  ✗ Error: ${e.message}`));
        console.log(chalk.dim('    Ensure the HyperNexus Go sidecar is running (typically via `hypernexus start`).'));
      }
      console.log('');
    });
  ...
  mcp
    ...
    .action(async () => {
      ...
      for (const s of startable) {
        const pidFile = `${process.env.HOME}/.hypernexus/mcp-pids/${s.name}.pid`;
        let alive = false;
        try {
          const fs = await import('fs');
          const pid = parseInt(fs.readFileSync(pidFile, 'utf8').trim());
      ...
      console.log(chalk.dim('  Use hypernexus mcp connect-all to spawn servers\n'));
    });
}
