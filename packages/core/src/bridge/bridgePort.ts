const DEFAULT_HYPERNEXUS_BRIDGE_PORT = 3001;
...
export function getDefaultBridgePort(): number {
    return DEFAULT_HYPERNEXUS_BRIDGE_PORT;
}

export function resolveBridgePort(env: NodeJS.ProcessEnv = process.env): number {
    return normalizePort(env.HYPERNEXUS_BRIDGE_PORT)
        ?? normalizePort(env.HYPERNEXUS_CORE_BRIDGE_PORT)
        ?? DEFAULT_HYPERNEXUS_BRIDGE_PORT;
}
