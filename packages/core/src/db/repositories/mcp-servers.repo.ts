...
import {
    HyperNexusMcpServerDiscoveryMetadata,
    HyperNexusMcpJsonConfig,
    HyperNexusMcpToolMetadata,
    loadHyperNexusMcpConfig,
    writeHyperNexusMcpConfig,
...
type DiscoveryResult = {
    metadata: HyperNexusMcpServerDiscoveryMetadata;
...
export class McpServersRepository {
    ...
    async clearMetadataCache(
        serverUuid: string,
    ): Promise<{ server: DatabaseMcpServer; metadata: HyperNexusMcpServerDiscoveryMetadata; toolCount: number }> {
        const server = await this.findByUuid(serverUuid);
        if (!server) {
            throw new Error(`MCP Server with UUID ${serverUuid} not found.`);
        }
        ...
        const metadata: HyperNexusMcpServerDiscoveryMetadata = {
    ...
    public async exportToolCache(metadataOverrides: Record<string, HyperNexusMcpServerDiscoveryMetadata> = {}): Promise<void> {
        try {
            ...
            const jsonOutput: HyperNexusMcpJsonConfig = {
                ...existingConfig,
                mcpServers: { ...existingConfig.mcpServers },
            };

            const toolsByServerUuid = new Map<string, HyperNexusMcpToolMetadata[]>();
    ...
    private async persistDiscoveredTools(serverUuid: string, tools: HyperNexusMcpToolMetadata[]): Promise<void> {
    ...
    private shouldPersistDiscoveredTools(metadata: HyperNexusMcpServerDiscoveryMetadata): boolean {
    ...
    private shouldThrottleBinaryReload(
        strategy: MetadataReloadStrategy,
        cachedMetadata: HyperNexusMcpServerDiscoveryMetadata | undefined,
        server: DatabaseMcpServer,
    ): boolean {
    ...
    private async discoverServerTools(server: DatabaseMcpServer): Promise<DiscoveryResult> {
            ...
            if (!server.command) {
                return {
                    metadata: buildFailureDiscoveryMetadata(
                        ...
                        'failed',
                        new Date().toISOString(),
                        'STDIO server is missing a command, so HyperNexus could not discover tools.',
                    ),
                    tools: [],
                    decision: 'binary-fresh',
                };
            ...
            if (!server.url) {
                return {
                    metadata: buildFailureDiscoveryMetadata(
                        ...
                        'failed',
                        new Date().toISOString(),
                        `${server.type} server is missing a URL, so HyperNexus could not discover tools.`,
                    ),
                    tools: [],
                    decision: 'binary-fresh',
                };
        ...
        try {
            ...
            client = new Client(
                { name: `hypernexus-discovery-${server.name}`, version: '1.0.0' },
                { capabilities: {} },
            );
