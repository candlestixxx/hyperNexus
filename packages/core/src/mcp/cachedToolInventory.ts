...
import { loadHyperNexusMcpConfig, type HyperNexusMcpServerEntry, type HyperNexusMcpToolMetadata } from './mcpJsonConfig.js';
...
function buildConfigSnapshot(configServers: Record<string, HyperNexusMcpServerEntry>): CachedMcpInventorySnapshot {
    ...
    servers.forEach((server) => {
        ...
        metadataTools.forEach((tool) => {
            const typedTool = tool as HyperNexusMcpToolMetadata;
            tools.push({
...
export async function getCachedToolInventory() {
    const config = await loadHyperNexusMcpConfig().catch(() => ({ mcpServers: {} }));
