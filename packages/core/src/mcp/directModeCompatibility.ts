...
export function getDirectModeCompatibilityTools(): Tool[] {
    return getCompatibilityToolDefinitions({
        descriptions: {
            run_code: 'HyperNexus-compatible alias for one-shot HyperNexus code execution without manually enabling Code Mode.',
            run_python: 'HyperNexus-compatible alias for HyperNexus sandboxed Python execution.',
            run_agent: 'HyperNexus-compatible autonomous tool-using agent loop backed by HyperNexus native LLM and tool execution surfaces.',
            save_memory: 'HyperNexus-compatible alias for persisting agent memory through HyperNexus native memory services.',
            search_memory: 'HyperNexus-compatible alias for searching HyperNexus native memory services.',
            save_script: 'HyperNexus-compatible alias for persisting reusable scripts in HyperNexus managed config.',
            save_tool_set: 'HyperNexus-compatible alias for saving the currently loaded HyperNexus session tools as a named tool set.',
            load_tool_set: 'HyperNexus-compatible alias for loading a saved HyperNexus tool set into the current session working set.',
            toolset_list: 'List HyperNexus-managed saved tool sets available to the current direct-mode session.',
            import_mcp_config: 'HyperNexus-compatible alias for importing MCP servers from Claude-style JSON config content.',
        },
    });
}
...
export async function tryHandleDirectModeCompatibilityTool(
    ...
    if (name === 'run_agent') {
        return await executeCompatibleRunAgent(
            ...
            agentRunner,
            delegatedToolCaller,
            'Agent runner not available in HyperNexus direct mode.',
        );
    }
    ...
    if (name === 'import_mcp_config') {
        return await executeCompatibleImportConfig(
            args,
            configImportService,
            'Config import service not available in HyperNexus direct mode.',
        );
    }
