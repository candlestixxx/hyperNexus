...
import type { HyperNexusMcpServerDiscoveryMetadata, HyperNexusMcpToolMetadata } from './mcpJsonConfig.js';
...
export function normalizeDiscoveredToolMetadata(rawTool: unknown): HyperNexusMcpToolMetadata | null {
...
export function buildBaseServerMetadata(server: Pick<DatabaseMcpServer, 'name' | 'type' | 'command' | 'args' | 'env' | 'url' | 'headers' | 'bearerToken'>): Omit<HyperNexusMcpServerDiscoveryMetadata, 'status' | 'toolCount' | 'tools'> {
    return {
        metadataVersion: 2,
...
export function buildBinaryDiscoveryMetadata(
    ...
    const tools = rawTools
        ...
        .filter((tool): tool is HyperNexusMcpToolMetadata => Boolean(tool));
...
export function buildFailureDiscoveryMetadata(
    server: Pick<DatabaseMcpServer, 'name' | 'description' | 'always_on' | 'type' | 'command' | 'args' | 'env' | 'url' | 'headers' | 'bearerToken'>,
    status: HyperNexusMcpServerDiscoveryMetadata['status'],
    discoveredAt: string,
    error: string,
): HyperNexusMcpServerDiscoveryMetadata {
    return enrichServerMetadata(server, {
...
export function hasReusableMetadataCache(
    metadata: HyperNexusMcpServerDiscoveryMetadata | null | undefined,
