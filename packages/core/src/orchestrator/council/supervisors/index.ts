...
import { HyperNexusSupervisor } from './hypernexus.js';

export function createSupervisor(config: SupervisorConfig): Supervisor {
  const { provider } = config;

  // Prefer HyperNexus-native supervisor if possible (it uses the shared LlmService)
  if (['openai', 'anthropic', 'google', 'gemini', 'grok', 'xai', 'deepseek', 'qwen', 'moonshot', 'kimi'].includes(provider)) {
    return new HyperNexusSupervisor(config);
  }
