...
import { getHyperNexusMcpJsoncPath, loadHyperNexusMcpConfig, stripJsonComments, writeHyperNexusMcpConfig } from '../mcp/mcpJsonConfig.js';
import {
...
async function writeToolPreferences(nextPreferences: ToolPreferences): Promise<ToolPreferences> {
    ...
    await writeHyperNexusMcpConfig({
        ...config,
...
export const mcpRouter = t.router({
    ...
    getJsoncEditor: publicProcedure.query(async () => {
        const jsoncPath = getHyperNexusMcpJsoncPath();
        try {
            ...
            if (errorCode === 'ENOENT') {
                const fallbackConfig = await loadHyperNexusMcpConfig();
                return {
                    path: jsoncPath,
    ...
    saveJsoncEditor: adminProcedure.input(z.object({
        ...
        await writeHyperNexusMcpConfig(parsed as never);
        return { ok: true };
    }),
    ...
    runServerTest: publicProcedure.input(z.object({
        ...
        const requestPayload = {
            ...
            via: input.targetKind === 'router' ? 'hypernexus-router' : 'direct-downstream',
        };
        ...
        if (input.operation === 'tools/call' && !input.toolName) {
            return {
                ...
                target: {
                    ...
                    displayName: input.targetKind === 'router' ? 'HyperNexus router' : input.serverName ?? 'Unknown downstream server',
        ...
        try {
            ...
            if (input.targetKind === 'router') {
                ...
                if (!aggregator) {
                    throw new Error('HyperNexus MCP router is not initialized.');
                }
            ...
            return {
                ...
                target: {
                    kind: input.targetKind,
                    displayName: input.targetKind === 'router' ? 'HyperNexus router' : input.serverName!,
                    serverName: input.serverName ?? null,
                    via: input.targetKind === 'router' ? 'hypernexus-router' : 'direct-downstream',
                },
                operation: input.operation,
