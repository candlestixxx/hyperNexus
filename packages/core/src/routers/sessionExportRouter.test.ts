...
describe('sessionExportRouter orchestrator base resolution', () => {
    ...
    it('exports sessions from the configured orchestrator base', async () => {
        process.env.HYPERNEXUS_ORCHESTRATOR_URL = 'http://127.0.0.1:5001';
    ...
    it('reports a clear error when importing without an orchestrator base', async () => {
        const configDir = mkdtempSync(`${os.tmpdir()}\\hypernexus-export-empty-`);
        ...
        expect(result.errors).toEqual([
            'Failed to import session sess-import-1: No HyperNexus Orchestrator base configured.',
        ]);
        expect(result.details).toEqual([
            {
                sessionId: 'sess-import-1',
                action: 'error',
                reason: 'No HyperNexus Orchestrator base configured.',
            },
        ]);
    });
});
