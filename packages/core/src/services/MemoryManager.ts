...
export class MemoryManager {
    ...
    public async recordToolOutcome(toolName: string, success: boolean, context: string) {
        ...
        await this.provider.addMemory(`Tool ${toolName}: ${success ? 'SUCCESS' : 'FAILURE'}. Context: ${context}`, {
    ...
    public async runMaintenance() {
        if (!this.initialized) await this.initialize();
        if (this.provider && this.provider.maintenance) await this.provider.maintenance();
    }
}
