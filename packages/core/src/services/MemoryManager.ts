...
export class MemoryManager {
  ...
  async initialize() {
    ...
    try {
      const { LanceDBStore, MemoryVectorStore } = await import('@hypernexus/memory');
      try {
      ...
      try {
        const { GraphMemory } = await import('@hypernexus/memory');
  ...
  public async indexCodebase(rootDir: string): Promise<number> {
    if (!this.initialized) await this.initialize();
    try {
      const { Indexer } = await import('@hypernexus/memory');
