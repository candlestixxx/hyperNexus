...
async function createTempRoot(): Promise<string> {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'hypernexus-session-import-'));
    tempRoots.push(root);
    return root;
}
...
describe('SessionImportService', () => {
    it('discovers session transcripts, stores imported sessions, derives memories, and writes instruction docs', async () => {
        ...
        await fs.writeFile(
            ...
            [
                JSON.stringify({ role: 'user', content: 'Use port 4000 for the HyperNexus control plane.' }),
                JSON.stringify({ role: 'assistant', content: 'Always prefer PowerShell on Windows for HyperNexus shell commands.' }),
            ].join('\n'),
            'utf-8',
        );
        ...
        expect(content).toContain('Always prefer PowerShell on Windows for HyperNexus shell commands.');
    });
    ...
    it('imports VS Code Copilot Chat home-directory sessions with UUID filenames', async () => {
        ...
        await fs.writeFile(
            ...
            [
                JSON.stringify({ request: { message: 'Remember that HyperNexus runs on port 4000.' } }),
    ...
    it('imports OpenAI or ChatGPT JSON history from explicit home-directory roots', async () => {
        ...
        await fs.writeFile(
            ...
            JSON.stringify([
                {
                    role: 'user',
                    content: 'Remember that HyperNexus should import durable chat history automatically.',
        ...
        expect(store.sessions[0]?.transcript).toContain('User: Remember that HyperNexus should import durable chat history automatically.');
    ...
    it('imports ChatGPT exported conversations.json mapping trees as per-conversation sessions', async () => {
        ...
        await fs.writeFile(
            ...
            JSON.stringify([
                {
                    id: 'conv-1',
                    title: 'HyperNexus stabilization plan',
        ...
        expect(store.sessions[0]?.title).toBe('HyperNexus stabilization plan');
    ...
    it('skips a discovered session file that disappears before import', async () => {
        const root = await createTempRoot();
        const hypernexusSessionDir = path.join(root, '.hypernexus', 'sessions');
        const sessionPath = path.join(hypernexusSessionDir, 'session_123.json');
        await fs.mkdir(hypernexusSessionDir, { recursive: true });
    ...
    it('ignores Cursor workspace metadata files that are not actual chat sessions', async () => {
        ...
        await fs.writeFile(
            ...
            JSON.stringify({ folder: 'C:\\Users\\hyper\\workspace\\hypernexus' }),
            'utf-8',
        );
    ...
    (sqliteAvailable ? it : it.skip)('imports Prism ledger and handoff entries from the local data.db store', async () => {
        ...
        prismDb
            ...
            .run(
                ...
                'hypernexus',
                'conversation-1',
                'Integrated the Prism session importer into HyperNexus.',
                ...
                JSON.stringify(['Prefer importing Prism ledger summaries into HyperNexus memory']),
        ...
        prismDb
            ...
            .run(
                'hypernexus',
                'Prism handoff summary for HyperNexus.',
                JSON.stringify(['Keep assimilating Prism features carefully']),
                JSON.stringify(['Use HyperNexus as the operator-facing control plane']),
                JSON.stringify(['prism', 'handoff']),
                'Current focus: bridge durable memory and imported sessions.',
                'main',
                7,
                JSON.stringify({ cwd: 'C:\\Users\\hyper\\workspace\\hypernexus' }),
        ...
        expect(store.sessions.map((session) => session.sourcePath)).toEqual(
            expect.arrayContaining([
                ...
                expect.stringContaining('#session_handoffs:hypernexus'),
            ]),
        );
        expect(store.sessions.map((session) => session.workingDirectory)).toContain('C:\\Users\\hyper\\workspace\\hypernexus');
        ...
        expect(prismLedger?.metadata).toMatchObject({
            ...
            prismImportance: 5,
            behavioralWarnings: ['Integrated the Prism session importer into HyperNexus.'],
        });
    ...
    (sqliteAvailable ? it : it.skip)('imports llm CLI conversations from logs.db', async () => {
        ...
        llmDb
            ...
            .run('conv-1', 'HyperNexus import strategy', 'anthropic/claude-3-5-sonnet');
        llmDb
            .prepare(`
            ...
            .run(
                ...
                'How should HyperNexus import llm CLI logs?',
        ...
        expect(String(store.sessions[0]?.transcript)).toContain('User: How should HyperNexus import llm CLI logs?');
