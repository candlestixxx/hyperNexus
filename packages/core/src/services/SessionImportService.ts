...
export class SessionImportService {
    ...
    constructor(
        ...
        this.store = options.store ?? new ImportedSessionStore(
            path.join(this.workspaceRoot, '.hypernexus', 'imported_sessions', 'archive'),
        );
        ...
        this.docsDir = path.join(this.workspaceRoot, '.hypernexus', 'imported_sessions', 'docs');
    ...
    private getDiscoveryRules(): SessionDiscoveryRule[] {
        ...
        const workspaceRules: SessionDiscoveryRule[] = [
            ...
            {
                sourceTool: 'hypernexus',
                roots: [path.join(this.workspaceRoot, '.hypernexus')],
                filePatterns: ['**/*.{md,txt,log,json,jsonl}'],
        ...
        return [
            ...
            {
                sourceTool: 'hypernexus',
                roots: [path.join(homeDir, '.hypernexus')],
                filePatterns: ['**/*.{md,txt,log,json,jsonl}'],
                fileNameHints: ['session', 'memory', 'handoff', 'history'],
            },
            {
    ...
    private isGeneratedImportPath(filePath: string): boolean {
        ...
        const ignoredRoots = [
            path.join(this.workspaceRoot, '.hypernexus', 'imported_sessions'),
            path.join(os.homedir(), '.hypernexus', 'imported_sessions'),
        ]
    ...
    private async analyzeImportedSession(session: NormalizedImportedSession): Promise<ImportedSessionAnalysis> {
        ...
        const prompt = `
You are HyperNexus's session-import memory extractor and archive-retention analyst.
...
Return JSON only as an object:
{
  "memories": [
    {
      "fact": "Use port 4000 for the HyperNexus control plane.",
