...
const SHELL_CATALOG: ShellCatalogEntry[] = [
    {
        ...
        smokeTestArgs: ['-NoLogo', '-NoProfile', '-Command', 'Write-Output hypernexus-ready'],
        notes: ['Preferred HyperNexus shell on Windows for general command execution and structured scripting.'],
    },
    {
        id: 'powershell',
    ...
    },
    {
        ...
        family: 'cmd',
        versionArgs: ['/d', '/c', 'ver'],
        smokeTestArgs: ['/d', '/c', 'echo hypernexus-ready'],
    ...
    },
    {
        ...
        smokeTestArgs: ['-lc', 'printf hypernexus-ready'],
        executableHints: [
    ...
    },
    {
        ...
        family: 'wsl',
        versionArgs: ['--version'],
        smokeTestArgs: ['-e', 'sh', '-lc', 'printf hypernexus-ready'],
...
const TOOL_CATALOG: BinaryCatalogEntry[] = [
    ...
    },
    {
        ...
        notes: ['Required for large parts of the HyperNexus stack and many MCP servers.'],
    },
    {
        id: 'pnpm',
        name: 'pnpm',
        ...
        notes: ['Primary package manager for this HyperNexus workspace.'],
    },
    {
        id: 'docker',
        name: 'Docker',
...
async function detectShell(entry: ShellCatalogEntry, deps: ExecutionEnvironmentDeps): Promise<DetectedExecutionShell> {
    ...
    const verified = Boolean(smokeOutput?.toLowerCase().includes('hypernexus-ready'));
    ...
    return {
        ...
        notes: verified
            ? [...(entry.notes ?? [])]
            : [...(entry.notes ?? []), 'Shell was found, but HyperNexus could not verify a simple smoke test.'],
    };
}
...
function buildSummary(
    ...
    if (preferredShell) {
        notes.push(`Prefer ${preferredShell.name} for default HyperNexus shell execution on this host.`);
    }
    ...
    if (!supportsPosixShell && platform === 'win32') {
        notes.push('No verified POSIX shell detected yet; HyperNexus should prefer PowerShell-compatible command plans.');
    }
