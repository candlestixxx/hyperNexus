...
export async function detectInstallSurfaceArtifacts(workspaceRoot: string = findMonorepoRoot()): Promise<InstallSurfaceArtifactStatus[]> {
    const chromiumBundlePath = resolveExistingRelativePath(workspaceRoot, [
        path.join('apps', 'hypernexus-extension', 'dist-chromium'),
        path.join('apps', 'extension', 'dist'),
        path.join('apps', 'hypernexus-extension', 'dist'),
    ]);

    const firefoxBundlePath = resolveExistingRelativePath(workspaceRoot, [
        path.join('apps', 'hypernexus-extension', 'dist-firefox'),
    ]);
    ...
    const [browserExtensionVersion, vscodeExtensionVersion] = await Promise.all([
        readPackageVersion(workspaceRoot, path.join('apps', 'hypernexus-extension', 'package.json')),
    ...
    return [
        {
            ...
            artifactKind: chromiumBundlePath === path.join('apps', 'hypernexus-extension', 'dist-chromium')
                ? 'Chromium unpacked bundle'
                : chromiumBundlePath === path.join('apps', 'extension', 'dist')
                    ? 'Legacy extension dist bundle'
                    : chromiumBundlePath === path.join('apps', 'hypernexus-extension', 'dist')
                        ? 'Generic hypernexus-extension dist bundle'
                        : null,
        ...
        },
        {
            ...
            detail: mcpConfigPath
                ? 'HyperNexus-managed MCP config source is present for dashboard sync and preview flows.'
                : 'No HyperNexus MCP config source file was detected yet.',
