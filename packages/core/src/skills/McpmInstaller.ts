...
import { Registry as McpServerRegistry } from '@hypernexus/mcp-registry';
...
export class McpmInstaller {
    ...
    async install(skillName: string): Promise<string> {
        ...
        if (mcpServer) {
            ...
            try {
                // Execute the internal hypernexus CLI command
                await this.runCommand(`npx hypernexus mcp install ${mcpServer.package}`);
