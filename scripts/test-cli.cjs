...
const HYPERNEXUS = 'node packages/cli/dist/cli/src/index.js';

let passed = 0, failed = 0;

function test(name, cmd, expectFn, timeout = 10000) {
  try {
    const out = execSync(`${HYPERNEXUS} ${cmd}`, { timeout, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
    if (expectFn(out)) {
      console.log(`  ✓ ${name}`);
      passed++;
    } else {
...
(async () => {
  console.log(`\n  HyperNexus CLI Integration Test (v1.0.0-alpha.40)\n`);
  ...
  test('about shows codename', 'about', o => o.includes('HYPERNEXUS'));
  ...
  test('info', 'info', o => o.includes('HyperNexus') || o.includes('System'));
