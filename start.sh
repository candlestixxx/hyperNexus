...
if command -v go &>/dev/null; then
	...
	(cd go && go build -ldflags "-X github.com/hypernexushq/hypernexus-go/internal/buildinfo.Version=${VER}" -buildvcs=false -o ../bin/hypernexus ./cmd/hypernexus 2>/dev/null) && echo "      ✓ bin/hypernexus built" || echo "      [WARN] Go build failed"
else
...
HYPERNEXUS_SKIP_BUILD="${HYPERNEXUS_SKIP_BUILD:-0}"
if [ "$HYPERNEXUS_SKIP_BUILD" != "1" ]; then
...
HYPERNEXUS_PORT="${HYPERNEXUS_PORT:-4000}"
